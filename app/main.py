...
<head>
  ...
  <style>
    ...
    button, .button {{ font: inherit; display: inline-block; padding: .8rem 1rem; }}
    .status {{ display:inline-block; padding:.2rem .5rem; border:1px solid currentColor; border-radius:.3rem; }}
    .step {{ border-top:1px solid #bbb; padding-top:1rem; margin-top:1.5rem; }}
    .expected {{ margin-left:1rem; }}
    .placeholder {{ border:1px dashed #888; padding:.75rem; margin:1rem 0; }}
...
def index() -> HTMLResponse:
    return page("IT-Notfall Orchestrator", """
...
def show_incident(incident_id: str) -> HTMLResponse:
    ...
    return page(incident["id"], f"""
      <h1>{escape(incident['title'])}</h1>
      <dl>
        <dt>Incident-ID</dt><dd><code>{escape(incident['id'])}</code></dd>
        <dt>Status</dt><dd><span class="status">{escape(incident['status'])}</span></dd>
        <dt>Angelegt</dt><dd>{escape(incident['created_at'])}</dd>
      </dl>
      <p><a class="button" href="/incidents/{escape(incident_id)}/checklist">{escape(checklist['id'])} – {escape(checklist['title'])}</a></p>
      <p><a href="/">Zurück</a></p>
    """)
...
def show_checklist(incident_id: str) -> HTMLResponse:
    ...
    body = [
        ...
        "<h2>Benötigt</h2>", render_list(checklist["needed"]),
        "<h2>Werkzeugstandort</h2>", "<p>" + " → ".join(escape(p) for p in checklist["tool_location"]) + "</p>",
        "<h2>Checkliste</h2>",
    ]
    for step in checklist["steps"]:
        ...
        if step.get("image_placeholder"):
            body.append('<div class="placeholder"><strong>Bildplatzhalter:</strong> ' + escape(step["image_placeholder"]) + '</div>')
        body.append('</section>')
    body += ["<h2>Abschluss</h2>", f"<p>{escape(checklist['completion'])}</p>", f'<p><a href="/incidents/{escape(incident_id)}">Zurück zum Incident</a></p>']
    return page(f"{checklist['id']} – {checklist['title']}", "".join(body))
